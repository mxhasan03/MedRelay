...
def test_audit_app_is_installed() -> None:
    assert django_apps.is_installed("apps.audit")


def test_audit_app_config_importable() -> None:
    config = django_apps.get_app_config("audit")
    assert config.name == "apps.audit"
