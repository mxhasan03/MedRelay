...
def test_accounts_app_is_installed() -> None:
    assert django_apps.is_installed("apps.accounts")


def test_accounts_app_config_importable() -> None:
    config = django_apps.get_app_config("accounts")
    assert config.name == "apps.accounts"
